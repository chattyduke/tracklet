...
def main(argv: "list[str] | None" = None) -> int:
    ...
    for _stale in ("residual.txt", "report.md", "overlay.png", "track.tdm"):
...
def _run_synthetic(args, out_dir: Path) -> int:
    ...
    # 7b) Emit the CCSDS TDM (RADEC) track alongside the residual, on success only. The epoch is the
    #     exposure MIDPOINT — computed HERE from scene.utc (exposure START) + exposure_s/2, the same
    #     instant render seals as the scored-truth epoch. In-memory measured RA/Dec only.
    start_utc = dt.datetime.fromisoformat(scene.utc.replace("Z", "+00:00"))
    mid_utc = start_utc + dt.timedelta(seconds=scene.exposure_s / 2.0)
    write_tdm(result, mid_utc, "TRACKLET-SYNTH", out_dir / "track.tdm")
...
def _run_real(args, out_dir: Path) -> int:
    ...
    # 8b) Emit the CCSDS TDM (RADEC) track alongside the residual, on success only. The epoch is the
    #     real exposure MIDPOINT (rt.exposure_mid_utc); the station is the observatory name. In-memory
    #     measured RA/Dec only — no truth read.
    write_tdm(
        result, rt.exposure_mid_utc, meta["observatory"]["name"], out_dir / "track.tdm"
    )
